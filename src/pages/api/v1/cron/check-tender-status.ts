...
export const GET: APIRoute = async () => {
    ...
    const checkTendersToClose = async () => {
        ...
        */
        ...
        // FIX: Clone currentDate before setting hours to prevent side effects
        const closingDate = new Date(currentDate);
        closingDate.setHours(16, 0, 0, 0); // Sets the time to 4 PM on the current day

        const tenders = await prisma.tender.findMany({
            where: {
                endDate: { lte: new Date(closingDate).toISOString() },
                status: 'open'
            },
        });
        return tenders
